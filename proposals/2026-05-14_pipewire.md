...
An alternative is to use a different crossplatform library which has fine
grained API for different backends. One such contender is [libsoundio](https://github.com/andrewrk/libsoundio).
The issue is that this library is not maintained, and does not support
PipeWire on linux, only PulseAudio and JACK.
